...
import { Edit2, Trash2, Archive, Eye, EyeOff } from 'lucide-react';
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredAccounts.map(acc => (
                    <Card key={acc.id} className={`border shadow-sm hover:shadow-md transition-all group relative ${acc.isArchived ? 'bg-slate-50/50 border-slate-200' : 'border-slate-100'}`}>
                        <CardContent className="p-4">
                            <div className="flex justify-between items-center mb-1">
                                <div className="flex gap-1.5 items-center">
                                    ...
                                    {acc.isArchived && (
                                        <Badge className="text-[9px] uppercase bg-slate-200 text-slate-600 hover:bg-slate-200 border-none px-1.5 h-4">
                                            Archivada
                                        </Badge>
                                    )}
                                </div>
                            ...
                            <h3 className={`font-semibold truncate ${acc.isArchived ? 'text-slate-400' : 'text-slate-700'}`} title={acc.name}>
                                {acc.name}
                            </h3>

                            {showBalance && (
                                <p className={`text-xl font-bold mt-1 ${acc.isArchived ? 'text-slate-400' : (acc.balance < 0 ? 'text-rose-600' : 'text-slate-900')}`}>
                            ...
                            {canEdit && (
                                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/80 p-1 rounded-md shadow-sm backdrop-blur-sm">
                                    ...
                                    {acc.hasTransactions ? (
                                        <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onEdit?.(acc)} title="Esta cuenta tiene movimientos. Solo se puede archivar.">
